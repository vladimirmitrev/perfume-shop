        ...
        var ready = function () {
            try {
                ...
                var displayBadge = function(badgeUrl, badgeClass, badgeAlt, badgeLink, utmParameter, urlParameter) {
                    ...
                    badgeWrapper.appendChild(banner);  if (!JotForm.enterprise) {
...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
    ...
    prePopulations: function (fields, isPrefill) {
        ...
        setTimeout(function(){
            if(!(document.getElementById('draftID') || (JotForm.replaceTagTest && JotForm.isEditMode()))){
                JotForm.runAllConditions();
            }
        }, 1500);
    ...
    checkCondition: function (condition, sourceField, sourceEvent) {
        ...
        if (conditionInfiniteLoop()) {
            if (JotForm.isConditionDebugMode) {
                console.log({ condition: condition, sourceField: sourceField }, 'IN CHECK CONDITION');
            }
            return;
        }
                ...
                setTimeout(function (calc) {
                    ...
                    if (!matchForThisResult[condition.action[0].resultField+subfield]) {
                        try {
                            ...
                            if (JotForm.replaceTagTest || !(document.getElementById('draftID') || JotForm.isEditMode())) {
                                JotForm.clearField(toBeClearField, subfield, dontTrigger);
                            }

                            if(document.getElementById('draftID') && !JotForm.replaceTagTest) {
                ...
                case 'inline':
                    ...
                    if (data.indexOf('|') > -1) {
                        ...
                        $$(selector).each(function (el) {
                            if (!el.value.empty() && JotForm.isVisible(el) && (!(['checkbox', 'radio'].indexOf(el.getAttribute('type')) > -1) || el.checked)) {
                                valArr.push(el.value);
                            }
   ...
   */
    ...
    initTextboxValidation: function () {
        $$('.form-textbox[data-minlength]').each(function (item) {
            item.validateTextboxMinsize = function () {
                ...
                var max_val = parseInt(item.readAttribute('maxlength')) || Infinity;
    ...
    /**
     * Prevent required product empty
     */
    handleRequiredProducts: function () {
        try {
            var field = $$('input[name="simple_fpc"]')
            var paymentField = field && field[0] ? field[0].up('.form-line') : null;
            if (paymentField !== null) {
                var productsElement = $$('.form-product-item');
                productsElement.each(function (item) {
                    item.select('select, input').each(function (elems) {
                        if (elems.hasAttribute('id') && elems.getAttribute('id').indexOf('quantity') > -1){
                            ['change', 'blur', 'keydown'].forEach(function (eventName) {
                                elems.addEventListener(eventName, function(){
                                    JotForm.preventRequiredProductEmpty();
                                })
                            })
                        }
                    });
            ...
            console.log('Required Product Prevent :: handleRequiredProducts ::', e);
        }
    },
    ...
    checkProductSelectionWithQuantity: function (productElement) {
        try {
            ...
            if (productElement.select('table').length > 0) {
                productElement.select('.form-subproduct-quantity').each(function (quantityEl) {
                    quantityCount += parseInt(quantityEl.value);
                    hasQuantity = true;
                })
            } else {
                productElement.select('select, input').each(function (inputs) {
            ...
            console.log('Required Product Prevent :: getEmptyRequiredProduct ::', e);
        }
    },

    /**
     * Check if product required, user should not send empty
     */
    preventRequiredProductEmpty: function () {
        var result = true;
        var field = $$('input[name="simple_fpc"]');
        var paymentField = field && field[0] ? field[0].up('.form-line') : null;
        try {
    ...
    isInputAlwaysHidden: function (input) {
        ...
        if (JotForm.payment === 'paypalpro' && input.up('#creditCardTable') && input.up('#creditCardTable').style.display === "none") {
            return true;
        }
        ...
        if(firstError) {
            ...
            if (!JotForm.noJump && erroredLine) {
                if (JotForm.browserIs.ios()) {
                    // ScrollIntoViewOptions does not working on the ios safari browser
                    window.parent.postMessage('scrollIntoView::' + document.querySelector('form').getAttribute('id'), '*');
                }
  ...
  */
    ...
    validator: function () {
        ...
        $A(JotForm.forms).each(function (form) { // for each JotForm form on the page
            ...
            var handleFormSubmit = function (e) {
                ...
                JotForm.sendFormOpenId(form, 'clientSubmitFinished');
  ...
  /**
    ...
    getOptionOtherInput: function(option) {
        ...
        var otherInputSelector = '.form-' + option.type + '-item';
        var parentWrapper = option.up(otherInputSelector);
        if (parentWrapper) {
    ...
    sendFormOpenId: function (form, type) {
        try {
            var formId = form.id;
            if (parseInt(formId, 10) % 10 !== 4) return;
            var openFormIdInput = form.querySelector('[name="formOpenId"]');
            if (!openFormIdInput) return;

            JotForm.createXHRRequest(JotForm.getAPIEndpoint() + '/form/' + form.id + '/event/' + openFormIdInput.value + '/' + type, 'post', {}, function cb(res) {
                console.log(res);
            }, function errCb(err) {
                console.log(err);
            });
...
function trackExecution(value) {
    $A(document.forms).each(function (form, index) {
        if (form.name === "form_" + form.id) {
            ...
            if (jsExecutionTrackerElement) {
                ...
                jsExecutionTrackerElement.value += '=>' + (value + indexString + ':' + (new Date()).getTime());
            } else {
